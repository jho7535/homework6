...
typedef struct Node {	//노드 구조체 정의
	int key;	//노드에 값을 저장할 변수
	struct Node* link;	//노드를 가리킬 포인터
} listNode;

typedef struct Head {	//연결리스트의 가장 앞부분 헤드 구조체 정의
	struct Node* first;	//노드를 가리킬 포인터
}headNode;
...
headNode* initialize(headNode* h) {	//헤드 노드를 초기화하는 함수
	...
	headNode* temp = (headNode*)malloc(sizeof(headNode));   //헤드 노드 포인터 temp를 선언하고 headNode 사이즈만큼 동적 할당
	temp->first = NULL; //헤드 노드가 가리키는 값 NULL로 초기화
	return temp;    //temp 반환
}
...
int freeList(headNode* h){	//모든 노드 메모리 해제하는 함수
	...
	while(p != NULL) {  //p가 NULL이 아닌 동안 반복
		prev = p;   //prev에 p를 대입
		p = p->link;	//p에 link에 저장되어있는 다음 노드를 대입
...
int insertNode(headNode* h, int key){	//노드에 값을 대입하는 함수
	...
	node->key = key;	//리스트 노드에 key 값 대입
	node->link = NULL;	//리스트 노드의 포인터를 NULL로 초기화

	if (h->first == NULL)	//헤드 노드의 포인터가 가리키는 노드가 없다면
	{
		h->first = node;	//헤드 노드의 포인터를 node로 초기화
		return 0;
	}

	listNode* n = h->first;	//리스트 노드 n을 선언하고 헤드 노드의 포인터 값 대입
	listNode* trail = h->first;	//리스트 노드 trail을 선언하고 헤드 노드의 포인터 값 대입
	...
	while(n != NULL) {	//n이 NULL 값이 아닌 동안
		if(n->key >= key) {	//리스트 노드 n의 key 값이 입력 받은 key 값보다 크거나 같으면
			...
			if(n == h->first) {	//리스트 노드 n이 헤드 노드의 포인터와 같다면
				h->first = node;	//헤드 노드의 포인터에 리스트 노드 node를 대입
				node->link = n;	//node의 포인터에 n을 대입
			} else { /* 중간이거나 마지막인 경우 */
				node->link = n;	//node의 포인터에 n을 대입
				trail->link = node;	//trail의 포인터에 node를 대입
			}
			return 0;
		}

		trail = n;	//trail의 포인터에 n을 대입
		n = n->link;	//n에 다음 노드를 대입
	}
...
int insertLast(headNode* h, int key) {	//노드에 값을 입력하고 가장 마지막 순서로 삽입하는 함수
	...
	node->link = NULL;	//node의 포인터를 NULL로 초기화
	...
	while(n->link != NULL) {	//n의 다음 노드가 NULL이 아닌 동안
		n = n->link;	//n에 다음 노드를 대입
	}
	n->link = node;	//n의 포인터에 node를 대입
	return 0;
}
...
int insertFirst(headNode* h, int key) {	//노드에 값을 입력하고 가장 처음 순서로 삽입하는 함수
	...
	node->link = h->first;	//node의 포인터에 헤드 노드의 포인터를 대입
 ...
 */
int deleteNode(headNode* h, int key) {	//노드에서 해당하는 key 값을 제거하는 함수
	...
	while(n != NULL) {	//n이 NULL 값이 아닌 동안
		if(n->key == key) {	//n의 key 값과 입력 받은 key 값이 같다면
			/* 첫 노드 앞쪽에 삽입해야할 경우 인지 검사 */
			if(n == h->first) {	//n과 헤드 노드의 포인터가 같다면
				h->first = n->link;	//헤드 노드의 포인터에 다음 노드 대입
			} else { /* 중간인 경우거나 마지막인 경우 */
				trail->link = n->link;	//trail의 포인터에 다음 노드 대입
			}
			free(n);	//n 메모리 해제
			return 0;
		}
...
int deleteLast(headNode* h) {	//마지막 노드를 삭제하는 함수
	...
	/* 마지막 노드까지 이동 */
	while(n->link != NULL) {	//n의 포인터가 NULL이 아닌 동안
		trail = n;	//trail에 n 대입
		n = n->link;	//n에 다음 노드 대입
	}

	/* n이 삭제되므로, 이전 노드의 링크 NULL 처리 */
	trail->link = NULL;	//trail의 포인터에 NULL 대입
	free(n);	//n 메모리 해제
...
int invertList(headNode* h) {	//노드의 순서를 역순으로 재배치하는 함수
	...
	listNode *n = h->first;		//리스트 노드 n을 선언하고 헤드 노드의 포인터로 초기화
	listNode *trail = NULL;	//리스트 노드 trail을 선언하고 NULL로 초기화
	listNode *middle = NULL;	//리스트 노드 middle을 선언하고 NULL로 초기화

	while(n != NULL){	//n이 NULL이 아닌 동안
		trail = middle;	//trail에 middle 대입
		middle = n;	//middle에 n 대입
		n = n->link;	//n에 n의 포인터 대입
		middle->link = trail;	//middle의 포인터에 trail 대입
	}
...
void printList(headNode* h) {	//리스트의 순서와 값을 차례대로 출력
